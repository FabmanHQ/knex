...
class Client extends EventEmitter {
  ...
  async acquireConnection() {
    ...
    try {
      ...
      connection.__knexAcquiredAfter = acquireStart;
      connection.__knexAcquiredAt = new Date();
      return connection;
    } catch (error) {
      ...
      if (error instanceof TimeoutError) {
        convertedError = new KnexTimeoutError(
          `Knex: Timeout acquiring a connection at ${acquireEnd.toISOString()} after ${
            acquireEnd - acquireStart
          }ms. The pool is probably full. ` +
            'Are you missing a .transacting(trx) call?',
          [...this.pool.used.map((c) => c.resource)],
          [...this.pool.free.map((c) => c.resource)]
        );
      }
  ...
  releaseConnection(connection) {
    connection.__knexReleasedAt = new Date();
